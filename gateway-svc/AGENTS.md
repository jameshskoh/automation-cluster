...
- Root/parent service folder: `gateway-svc`, packaging=pom, no source code.
- All submodules drop the `-svc` suffix: `gateway-domain`, `gateway-controller`, etc.
- `adapters/in/` and `adapters/out/` are filesystem-only groupings, not Maven modules — actual module names stay flat (`gateway-controller`, not `adapters-in-gateway-controller`).
...
Base package: `com.jameshskoh.gateway`

| Module | Package |
|---|---|
| `gateway-domain` | `com.jameshskoh.gateway.domain` |
| `gateway-application` | `com.jameshskoh.gateway.application` (`.in`, `.out` for ports) |
| `gateway-common` | `com.jameshskoh.gateway.common` |
| `gateway-controller` | `com.jameshskoh.gateway.adapter.in.controller` |
| `gateway-scheduler` | `com.jameshskoh.gateway.adapter.in.scheduler` |
| `gateway-consumer` | `com.jameshskoh.gateway.adapter.in.consumer` |
| `gateway-repository` | `com.jameshskoh.gateway.adapter.out.repository` |
| `gateway-client` | `com.jameshskoh.gateway.adapter.out.client` |
| `gateway-producer` | `com.jameshskoh.gateway.adapter.out.producer` |
| `gateway-bootstrap` | `com.jameshskoh.gateway` (no `.bootstrap` suffix; `@SpringBootApplication` class here) |
| `gateway-test` | mirrors the package of the module under test (no `.test` segment) |

## Test module structure

- All test code lives in `gateway-test`. No other module has a `src/test` directory.
- `gateway-test` has a compile-scope dependency on every other module (including bootstrap). This is the one intentional exception to the inward-dependency rule — `gateway-test` is a leaf consumer, never part of the runtime artifact.
...
- Always run the whole suite: `./mvnw test -pl gateway-test`. Use `-Dtest=ClassName` or IDE filtering to scope locally.
...
- `@SpringBootApplication` sits at `com.jameshskoh.gateway` (bare root) — default scan covers every module's sub-package automatically. No explicit `@ComponentScan` needed.
...
./mvnw test -pl gateway-test

# Start the application
./mvnw spring-boot:run -pl gateway-bootstrap
```
